...
-- ----------------------------------------------------------------------------------------
-- CTB00001- GIS_TB01S008_CURSOS
--
-- Criar esta tabela com as colunas:
--
-- codigo        numeric  (5,0)    Not Null PK CL  Código do curso
-- nome          varchar  (90)         Null        Nome do curso
-- obs           varchar  (72)         Null        Comentário (Observação)
--
-- Atenção:
-- Declarar a Chave Primária (Primary Key, ou Identificador Único, ou apenas PK) através de
-- um constraint de nome PK_GIS_TB01S008_CURSOS.
--
...
--
-- Definir um constraint de nome CK_GIS_TB01S008_CURSOS_CODIGO  para  a  coluna  CODIGO,  a 
...
If  exists 
 (Select *
   ...
   Where object_id  =  OBJECT_ID(N'dbo.CK_GIS_TB01S008_CURSOS') 
     and parent_object_id = OBJECT_ID(N'dbo.GIS_TB01S008_CURSOS'))
   ALTER TABLE  dbo.GIS_TB01S008_CURSOS
      DROP CONSTRAINT CK_GIS_TB01S008_CURSOS;
IF  EXISTS
  (select *
    From  sys.objects
    Where object_id = OBJECT_ID(N'dbo.GIS_TB01S008_CURSOS') 
      and type in (N'U'))
   DROP TABLE dbo.GIS_TB01S008_CURSOS;
--
-- ----------------------------------------------------------------------------------------
-- Criação da tabela.
-- ----------------------------------------------------------------------------------------
--
CREATE TABLE dbo.GIS_TB01S008_CURSOS (
  codigo       numeric  (5, 0)  NOT NULL ,
  nome         varchar  (90)        NULL ,
  obs          varchar  (72)        NULL ,
  CONSTRAINT PK_GIS_TB01S008_CURSOS PRIMARY KEY CLUSTERED 
   (codigo  asc)
   WITH
    FILLFACTOR = 10,
  CONSTRAINT CK_GIS_TB01S008_CURSOS_codigo CHECK
   (codigo > 0)
);
--
-- ----------------------------------------------------------------------------------------
