...
-- ----------------------------------------------------------------------------------------
-- CTB00001- GIS_TB08S002_CIDADE_DATAS
--
...
-- um constraint de nome PK_GIS_TB08S002_CIDADE_DATAS.
--
...
--
-- Definir  um  constraint  de  nome  CK_GIS_TB08S002_CIDADE_DATAS_COD_PAIS  para  a coluna
-- COD_PAIS, a qual deve admitir valores entre 1 e 250 (inclusive).
--
-- Definir um constraint de nome CK_GIS_TB08S002_CIDADE_DATAS_SEQ para a coluna SEQ, a qual
-- qual deve admitir valores maiores do que 0 (zero).
--
-- Definir  um  constraint  de  nome  FK_GIS_TB08S002_CIDADE_DATAS_CIDADE  para as  colunas
...
If  exists 
 (Select *
   ...
   Where object_id  =  OBJECT_ID(N'dbo.CK_GIS_TB08S002_CIDADE_DATAS') 
     and parent_object_id = OBJECT_ID(N'dbo.GIS_TB08S002_CIDADE_DATAS'))
   ALTER TABLE  dbo.GIS_TB08S002_CIDADE_DATAS
      DROP CONSTRAINT CK_GIS_TB08S002_CIDADE_DATAS;
IF  EXISTS
  (select *
    From  sys.objects
    Where object_id = OBJECT_ID(N'dbo.GIS_TB08S002_CIDADE_DATAS') 
      and type in (N'U'))
   DROP TABLE dbo.GIS_TB08S002_CIDADE_DATAS;
--
-- ----------------------------------------------------------------------------------------
-- Criação da tabela.
-- ----------------------------------------------------------------------------------------
--
CREATE TABLE dbo.GIS_TB08S002_CIDADE_DATAS (
  ...
  obs            varchar (72)        Null ,
  Constraint PK_GIS_TB08S002_CIDADE_DATAS Primary Key Clustered
   (cod_pais asc, uf asc, cidade asc, data_comem asc, seq asc)
   WITH
    FILLFACTOR = 10,
  CONSTRAINT CK_GIS_TB08S002_CIDADE_DATAS_cod_pais CHECK
   (cod_pais between 1 and 250),
  CONSTRAINT CK_GIS_TB08S002_CIDADE_DATAS_seq CHECK
   (seq > 0),
  CONSTRAINT FK_GIS_TB08S002_CIDADE_DATAS_cidade FOREIGN KEY (cod_pais, uf, cidade)
