...
-- ----------------------------------------------------------------------------------------
-- CTB00001- GIS_TB08S007_UF_GOVERNANTES
--
...
-- um constraint de nome PK_GIS_TB08S007_UF_GOVERNANTES.
--
...
--
-- Definir  um  constraint  de nome CK_GIS_TB08S007_UF_GOVERNANTES_COD_PAIS  para a  coluna
-- COD_PAIS, a qual deve admitir valores entre 1 e 250 (inclusive).
--
-- Definir um constraint de nome CK_GIS_TB08S007_UF_GOVERNANTES_SEXO para a coluna SEXO,  a
-- qual deve admitir os seguintes valores: F, M (sendo, F= Feminino e M= Masculino).
-- Validar tanto maiúsculas como minúsculas.
--
-- Definir um constraint de nome CK_GIS_TB08S007_UF_GOVERNANTES_SEQ para  a coluna  SEQ,  a
-- qual deve admitir valores maiores do que 0 (zero).
--
-- Definir um constraint de nome CK_GIS_TB08S007_UF_GOVERNANTES_TIPO para a coluna TIPO,  a
-- qual deve admitir os valores:
-- D, M, P, R (sendo, D= Ditadura, M= Monarquia, P= Presidencialismo e R= Parlamentarismo).
-- Validar tanto maiúsculas como minúsculas.
--
-- Definir um constraint de nome CK_GIS_TB08S007_UF_GOVERNANTES_COND para a coluna COND,  a
-- qual deve admitir os valores:
-- C, E, I, N, V.
-- (Sendo: C= Colégio Eleitoral, E= Eleito, I= Interino, N= Nomeado e V= Vice-presidente).
-- Validar tanto maiúsculas como minúsculas.
--
-- Definir um constraint de nome CK_GIS_TB08S007_UF_GOVERNANTES_COD_PAIS_NASC para a coluna
-- COD_PAIS_NASC, a qual deve admitir valores entre  1 e 250 (inclusive).
--
-- Definir um constraint de nome  CK_GIS_TB08S007_UF_GOVERNANTES_COD_PAIS_FALEC  para a co-
-- luna COD_PAIS_FALEC, a qual deve admitir valores entre 1 e 250 (inclusive).
--
-- Definir um constraint  de nome  FK_GIS_TB08S007_UF_GOVERNANTESS_CIDADE_NASC  para as co-
...
--
-- Definir  um constraint  de nome  FK_GIS_TB08S007_UF_GOVERNANTES_CIDADE_FALEC para as co-
-- lunas  FALEC_COD_PAIS, FALEC_UF e FALEC_CIDADE,  que são a chave estrangeira concatenada
-- (Foreign Key ou FK).
-- Elas referenciam as colunas COD_PAIS, UF e CIDADE da tabela GIS_TB07S001_CIDADES.
--
-- ----------------------------------------------------------------------------------------
...
If  exists 
 (Select *
   ...
   Where object_id  =  OBJECT_ID(N'dbo.CK_GIS_TB08S007_UF_GOVERNANTES') 
     and parent_object_id = OBJECT_ID(N'dbo.GIS_TB08S007_UF_GOVERNANTES'))
   ALTER TABLE  dbo.GIS_TB08S007_UF_GOVERNANTES
      DROP CONSTRAINT CK_GIS_TB08S007_UF_GOVERNANTES;
IF  EXISTS
  (select *
    From  sys.objects
    Where object_id = OBJECT_ID(N'dbo.GIS_TB08S007_UF_GOVERNANTES') 
      and type in (N'U'))
   DROP TABLE dbo.GIS_TB08S007_UF_GOVERNANTES;
--
-- ----------------------------------------------------------------------------------------
-- Criação da tabela.
-- ----------------------------------------------------------------------------------------
--
CREATE TABLE dbo.GIS_TB08S007_UF_GOVERNANTES (
  ...
  Constraint PK_GIS_TB08S007_UF_GOVERNANTES Primary Key Clustered
   (cod_pais asc, uf asc, periodo_de asc, periodo_ate asc, seq asc)
   WITH
    FILLFACTOR = 10,
  CONSTRAINT CK_GIS_TB08S007_UF_GOVERNANTES_cod_pais CHECK
   (cod_pais between 1 and 250),
  CONSTRAINT CK_GIS_TB08S007_UF_GOVERNANTES_sexo CHECK
   (sexo in ('f', 'm', 'F', 'M')),
  CONSTRAINT CK_GIS_TB08S007_UF_GOVERNANTES_seq CHECK
   (seq > 0),
  CONSTRAINT CK_GIS_TB08S007_UF_GOVERNANTES_tipo CHECK
   (tipo in ('D', 'M', 'P', 'R')),
  CONSTRAINT CK_GIS_TB08S007_UF_GOVERNANTES_cond CHECK
   (condição in ('C', 'E', 'I', 'N', 'V')),
  CONSTRAINT CK_GIS_TB08S007_UF_GOVERNANTES_cod_pais_nasc CHECK
   (nasc_cod_pais between 1 and 250),
  CONSTRAINT CK_GIS_TB08S007_UF_GOVERNANTES_cod_pais_falec CHECK
   (falec_cod_pais between 1 and 250),
  CONSTRAINT FK_GIS_TB08S007_UF_GOVERNANTES_cidade_nasc FOREIGN KEY (nasc_cod_pais, nasc_uf, nasc_cidade)
    References dbo.GIS_TB07S001_CIDADES (cod_pais, uf, cidade),
  CONSTRAINT FK_GIS_TB08S007_UF_GOVERNANTES_cidade_falec FOREIGN KEY (falec_cod_pais, falec_uf, falec_cidade)
    References dbo.GIS_TB07S001_CIDADES (cod_pais, uf, cidade)
);
--
-- ----------------------------------------------------------------------------------------
