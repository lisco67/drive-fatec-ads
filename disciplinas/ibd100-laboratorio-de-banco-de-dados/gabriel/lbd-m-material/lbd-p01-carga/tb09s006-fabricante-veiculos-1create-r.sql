...
-- ----------------------------------------------------------------------------------------
-- CTB00001- GIS_TB09S006_FABRICANTE_VEICULOS
--
-- Criar esta tabela com as colunas:
--
-- codigo          numeric (5, 0)   Not Null PK CL  Código do fabricante
-- raz_social      varchar (95)         Null        Nome do fabricante- Razão Social
...
-- um constraint de nome PK_GIS_TB09S006_FABRICANTE_VEICULOS.
--
...
--
-- Definir um constraint de nome CK_GIS_TB09S006_FABRICANTE_VEICULOS_COD_PAIS para a coluna
-- END_COD_PAIS, a qual deve admitir valores entre 1 e 250 (inclusive).
--
-- Definir um constraint de nome FK_GIS_TB09S006_FABRICANTE_VEICULOS_END_CIDADE para as co-
-- lunas  END_COD_PAIS, END_UF e END_CIDADE,  que são a  chave estrangeira concatenada (Fo-
-- reign Key ou FK).
...
If  exists 
 (Select *
   ...
   Where object_id  =  OBJECT_ID(N'dbo.CK_GIS_TB09S006_FABRICANTE_VEICULOS') 
     and parent_object_id = OBJECT_ID(N'dbo.GIS_TB09S006_FABRICANTE_VEICULOS'))
   ALTER TABLE  dbo.GIS_TB09S006_FABRICANTE_VEICULOS
      DROP CONSTRAINT CK_GIS_TB09S006_FABRICANTE_VEICULOS;
IF  EXISTS
  (select *
    From  sys.objects
    Where object_id = OBJECT_ID(N'dbo.GIS_TB09S006_FABRICANTE_VEICULOS') 
      and type in (N'U'))
   DROP TABLE dbo.GIS_TB09S006_FABRICANTE_VEICULOS;
--
-- ----------------------------------------------------------------------------------------
-- Criação da tabela.
-- ----------------------------------------------------------------------------------------
--
CREATE TABLE dbo.GIS_TB09S006_FABRICANTE_VEICULOS (
  ...
  obs              varchar (72)        NULL ,
  Constraint PK_GIS_TB09S006_FABRICANTE_VEICULOS Primary Key Clustered
   (codigo asc)
   WITH
    FILLFACTOR = 10,
  CONSTRAINT CK_GIS_TB09S006_FABRICANTE_VEICULOS_cod_pais CHECK
   (end_cod_pais between 1 and 250),
  CONSTRAINT FK_GIS_TB09S006_FABRICANTE_VEICULOS_end_cidade FOREIGN KEY
   (end_cod_pais, end_uf, end_cidade)
