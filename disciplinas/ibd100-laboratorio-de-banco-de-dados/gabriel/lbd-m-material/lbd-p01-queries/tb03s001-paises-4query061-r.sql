-- ----------------------------------------------------------------------------------------
-- Gabriel Issa Shammas (GIS) - 01 de janeiro de 2000 a 31 de dezembro de 2020
-- ----------------------------------------------------------------------------------------
-- QUERY061- GIS_TB03S001_PAISES
--
-- Qual é a área média dos países da América e Ásia?
-- Atenção: Usar o predicado de lista no Select.
-- Atenção: Usar o predicado de igualdade no Subselect.
-- Atenção: Esta é a mesma query anterior, mas com uma terceira solução.
--
-- ----------------------------------------------------------------------------------------
-- Atenção. Indicação do BD a ser usado. Se for o caso, mude o nome do Banco.
-- ----------------------------------------------------------------------------------------
--
USE BDGIS;
--
-- ----------------------------------------------------------------------------------------
-- Seleção de todos os dados para visualização do conteúdo da tabela.
-- ----------------------------------------------------------------------------------------
--
Select avg(ext_km2) 'Área média dos países da América e Ásia'
  From dbo.GIS_TB03S001_PAISES
  Where cod_continente IN           -- Predicado IN
     (Select codigo                     -- Subselect
        From dbo.GIS_TB01S005_CONTINENTE
        Where nome = 'América'            -- Predicado de igualdade
           or nome = 'Ásia (Eurásia)');
--
-- ----------------------------------------------------------------------------------------
-- Comentários.
-- ----------------------------------------------------------------------------------------
--
-- No comando SELECT, a tupla também pode ser elaborada com funções.
--
-- O SUBSELECT é um comando SELECT dentro de outro comando SELECT.
--
-- A execução do SUBSELECT se dá do final para o começo.
--
-- Nesta solução, o SUBSELECT utiliza o predicado de igualdade.
--
-- ----------------------------------------------------------------------------------------
-- Fim TB03S001 QUERY061
-- ----------------------------------------------------------------------------------------
--
--
