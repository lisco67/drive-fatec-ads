...
--
-- Definir  um constraint de nome  CK_GIS_TB04S004_PAISES_IDIOMAS_COD_PAIS  para  a  coluna
-- COD_PAIS, a qual deve admitir valores entre 1 e 250 (inclusive).
--
-- Definir  um constraint  de nome CK_GIS_TB04S004_PAISES_IDIOMAS_SEQ para  a  coluna  SEQ,
-- a qual deve admitir valores maiores do que 0 (zero).
--
-- Definir  um constraint  de nome FK_GIS_TB04S004_PAISES_IDIOMAS_COD_PAIS  para  a  coluna
...
If  exists 
 (Select *
   ...
   Where object_id  =  OBJECT_ID(N'dbo.CK_GIS_TB04S004_PAISES_IDIOMAS') 
     and parent_object_id = OBJECT_ID(N'dbo.GIS_TB04S004_PAISES_IDIOMAS'))
   ALTER TABLE  dbo.GIS_TB04S004_PAISES_IDIOMAS
      DROP CONSTRAINT CK_GIS_TB04S004_PAISES_IDIOMAS;
IF  EXISTS
  (select *
    From  sys.objects
    Where object_id = OBJECT_ID(N'dbo.GIS_TB04S004_PAISES_IDIOMAS') 
      and type in (N'U'))
   DROP TABLE dbo.GIS_TB04S004_PAISES_IDIOMAS;
--
-- ----------------------------------------------------------------------------------------
-- Criação da tabela.
-- ----------------------------------------------------------------------------------------
--
CREATE TABLE dbo.GIS_TB04S004_PAISES_IDIOMAS (
  cod_pais     numeric (3, 0) NOT NULL ,
  seq          numeric (2, 0) NOT NULL ,
  idioma       varchar (45)       NULL ,
  obs          varchar (72)       NULL ,
  Constraint PK_GIS_TB04S004_PAISES_IDIOMAS Primary Key Clustered
   (cod_pais asc, seq asc)
   WITH
    FILLFACTOR = 10,
  CONSTRAINT CK_GIS_TB04S004_PAISES_IDIOMAS_cod_pais CHECK
   (cod_pais between 1 and 250),
  CONSTRAINT CK_GIS_TB04S004_PAISES_IDIOMAS_seq CHECK
   (seq > 0),
  CONSTRAINT FK_GIS_TB04S004_PAISES_IDIOMAS_cod_pais FOREIGN KEY (cod_pais)
