...
-- ----------------------------------------------------------------------------------------
-- CTB00001- GIS_TB06S004_UF_POPULACAO
--
...
-- um constraint de nome PK_GIS_TB06S004_UF_POPULACAO.
--
...
--
-- Definir  um  constraint  de  nome  CK_GIS_TB06S004_UF_POPULACAO_COD_PAIS  para a  coluna
-- COD_PAIS, a qual deve admitir valores entre 1 e 250 (inclusive).
--
-- Definir um constraint de nome CK_GIS_TB06S004_UF_POPULACAO_SEQ para a coluna SEQ, a qual
-- deve admitir valores maiores do que 0 (zero).
--
-- Definir um constraint de nome FK_GIS_TB06S004_UF_POPULACAO_UF  para  as colunas COD_PAIS
-- e UF, que são a chave estrangeira concatenada (Foreign Key ou FK).
...
If  exists 
 (Select *
   ...
   Where object_id  =  OBJECT_ID(N'dbo.CK_GIS_TB06S004_UF_POPULACAO') 
     and parent_object_id = OBJECT_ID(N'dbo.GIS_TB06S004_UF_POPULACAO'))
   ALTER TABLE  dbo.GIS_TB06S004_UF_POPULACAO
      DROP CONSTRAINT CK_GIS_TB06S004_UF_POPULACAO;
IF  EXISTS
  (select *
    From  sys.objects
    Where object_id = OBJECT_ID(N'dbo.GIS_TB06S004_UF_POPULACAO') 
      and type in (N'U'))
   DROP TABLE dbo.GIS_TB06S004_UF_POPULACAO;
--
-- ----------------------------------------------------------------------------------------
-- Criação da tabela.
-- ----------------------------------------------------------------------------------------
--
CREATE TABLE dbo.GIS_TB06S004_UF_POPULACAO (
  cod_pais     numeric (3, 0)  NOT NULL ,
  uf           char    (3)     NOT NULL ,
  ano          numeric (4, 0)  NOT NULL ,
  população    numeric (9, 0)      NULL ,
  obs          varchar (72)        NULL ,
  Constraint PK_GIS_TB06S004_UF_POPULACAO Primary Key Clustered
  (cod_pais asc, uf asc, ano asc)
   WITH
    FILLFACTOR = 10,
  CONSTRAINT CK_GIS_TB06S004_UF_POPULACAO_cod_pais CHECK
   (cod_pais between 1 and 250),
  CONSTRAINT CK_GIS_TB06S004_UF_POPULACAO_ano CHECK
   (ano > 0),
  CONSTRAINT FK_GIS_TB06S004_UF_POPULACAO_uf FOREIGN KEY (cod_pais, uf)
