...
-- val_IDH         decimal (4, 3)   Not Null PK CL  Valor do IDH
-- obs             varchar (72)         Null        Comentário (Observação)
--
-- Atenção:
-- Declarar a Chave Primária (Primary Key, ou Identificador Único, ou apenas PK) através de
-- um constraint de nome PK_GIS_TB06S007_UF_IDH.
-- As colunas são: cod_pais, uf, ano_IDH.
--
...
--
-- Definir  um  constraint  de  nome  CK_GIS_TB06S007_UF_IDH_COD_PAIS  para  a   coluna
-- COD_PAIS, a qual deve admitir valores entre 1 e 250 (inclusive).
--
-- Definir um constraint de nome CK_GIS_TB06S007_UF_IDH_ANO_IDH para a  coluna ANO_IDH,
-- a qual deve admitir valores maiores do que 0 (zero).
--
-- Definir um constraint de nome CK_GIS_TB06S007_UF_IDH_VAL_IDH para a  coluna VAL_IDH,
-- a qual deve admitir valores menores ou iguais a 1.
--
-- Definir um constraint de nome  FK_GIS_TB06S007_UF_IDH  para as colunas  COD_PAIS  e  UF,
...
If  exists 
 (Select *
   ...
   Where object_id  =  OBJECT_ID(N'dbo.CK_GIS_TB06S007_UF_IDH') 
     and parent_object_id = OBJECT_ID(N'dbo.GIS_TB06S007_UF_IDH'))
   ALTER TABLE  dbo.GIS_TB06S007_UF_IDH
      DROP CONSTRAINT CK_GIS_TB06S007_UF_IDH;
IF  EXISTS
  (select *
    From  sys.objects
    Where object_id = OBJECT_ID(N'dbo.GIS_TB06S007_UF_IDH') 
      and type in (N'U'))
   DROP TABLE dbo.GIS_TB06S007_UF_IDH;
--
-- ----------------------------------------------------------------------------------------
-- Criação da tabela.
-- ----------------------------------------------------------------------------------------
...
if exists
 (select *
   from dbo.sysobjects
   where id = object_id(N'dbo.GIS_TB06S007_UF_IDH')
   and OBJECTPROPERTY(id, N'IsUserTable') = 1)
DROP TABLE dbo.GIS_TB06S007_UF_IDH;
--
CREATE TABLE dbo.GIS_TB06S007_UF_IDH (
  cod_pais      numeric (3, 0) NOT NULL ,
  uf            char    (3)    NOT NULL ,
  ano_idh       numeric (4, 0) NOT NULL ,
  val_idh       decimal (4, 3)     NULL ,
  obs           varchar (72)       NULL ,
  Constraint PK_GIS_TB06S007_UF_IDH Primary Key Clustered
   (cod_pais asc, uf asc, ano_idh asc)
   WITH
    FILLFACTOR = 10,
  Constraint CK_GIS_TB06S007_UF_IDH_cod_pais Check
   (cod_pais between 1 and 250),
  Constraint CK_GIS_TB06S007_UF_IDH_ano_idh Check
   (ano_idh > 0),
  Constraint CK_GIS_TB06S007_UF_IDH_val_idh Check
   (val_idh <= 1),
  CONSTRAINT FK_GIS_TB06S007_UF_IDH_uf FOREIGN KEY (cod_pais, uf)
